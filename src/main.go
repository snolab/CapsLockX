...
func clxmouse(act func()) func() {
	unA := turboTap(hotkey.KeyA,
		func(k int, taps int) { r := int(math.Log2(float64(taps))); robotgo.Move(-r, 0) },
		act)
	unD := turboTap(hotkey.KeyD,
		func(k int, taps int) { r := int(math.Log2(float64(taps))); robotgo.Move(r, 0) },
		act)
	unW := turboTap(hotkey.KeyW,
		func(k int, taps int) { r := int(math.Log2(float64(taps))); robotgo.Move(0, -r) },
		act)
	unS := turboTap(hotkey.KeyS,
		func(k int, taps int) { r := int(math.Log2(float64(taps))); robotgo.Move(0, r) },
		act)
	// TODO HOLD
	unE := turboTap(hotkey.KeyE,
		func(k int, taps int) { robotgo.Click() },
		act)
	unQ := turboTap(hotkey.KeyQ,
		func(k int, taps int) { robotgo.Click("right") },
		act)
	//
