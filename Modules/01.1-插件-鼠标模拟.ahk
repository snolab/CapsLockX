...
CursorShapeChangedQ()
{
    ...
    If(lA_Cursor == CursorHandleGet())Return 0lA_Cursor := CursorHandleGet()Return 1}sign(v)
{
...
鼠动(){
    ...
    ; 撞到屏幕边角就停下来
    if (TMouse_StopAtScreenEdge) {
        If(xa == xb)鼠速横 := 0If(ya == yb)鼠速纵 := 0xb := xa, yb := ya}    ; 对区域切换粘附
...
ScrollMsg2(msg, zDelta)
{
    ...
    If(GetKeyState("Shift", "p"))wParam := wParam | 0x4If(GetKeyState("Ctrl", "p"))wParam := wParam | 0x8PostMessage, msg, %wParam%, %lParam%, %fcontrol%, ahk_id %wid%}ScrollMsg(msg, zDelta)
{
    wParam := zDelta << 16
...
轮动(){
    ; 在非CapsLockX模式下停止
    if (!(CapsLockXMode || 轮自横 || 轮自纵))Return 轮动终()现刻 := TM_QPC()    ; 计算用户操作时间
轮时左 := dt(轮刻左, 现刻), 轮时右 := dt(轮刻右, 现刻)轮时上 := dt(轮刻上, 现刻), 轮时下 := dt(轮刻下, 现刻)    ; 计算加速度
轮加横 := ma(轮时右 - 轮时左) * TMouse_WheelSpeedRatio * TMouse_DPIRatio轮加纵 := ma(轮时下 - 轮时上) * TMouse_WheelSpeedRatio * TMouse_DPIRatio    ; RF 同时按下相当于中键（同时也会取消轮自动）
If(轮刻上 && 轮刻下 && Abs(轮时上 - 轮时下) < 1) {
        if(轮自刻)Return 轮动终()轮动终()SendInput {MButton Down}KeyWait, rKeyWait, fSendInput {MButton Up}Return}    ; 计算速度
轮速纵 := Friction(轮速纵 + 轮加纵, 轮加纵)轮速横 := Friction(轮速横 + 轮加横, 轮加横)    ; 处理自动滚动
...
if(轮自刻 <= 现刻 && 1000 < A_TimeIdlePhysical && A_TimeIdlePhysical < 30 * 60 * 1000) {
    ...
    ; 速度归 0 时，结束定时器
    if ( !轮速纵 && !轮速横 && !轮自刻)Return 轮动终()    ; 处理移动
轮差纵 += 轮速纵轮差横 += 轮速横if ((轮差纵 | 0) != 0) {
        if (TMouse_SendInputAPI && A_PtrSize == 4) ; 这API只能32位环境下用
        SendInput_MouseMsg32(0x0800, -轮差纵) ; 0x0800/*MOUSEEVENTF_WHEEL*/
        ElseScrollMsg2(0x20A, -轮差纵)轮差纵 -= 轮差纵 | 0}if ((轮差横 | 0) != 0) {
        if (TMouse_SendInputAPI && A_PtrSize == 4) ; 这API只能32位环境下用
        SendInput_MouseMsg32(0x1000, 轮差横) ; 0x1000/*MOUSEEVENTF_HWHEEL*/
        ElseScrollMsg2(0x20E, 轮差横) ; 在64位下用的是低性能的……轮差横 -= 轮差横 | 0}}#if CapsLockXMode
