...
if (!!CapsLockXConfigPath){
    ...
    global T_UseScrollLockLight := CapsLockX_Config("Core", "T_UseScrollLockLight", 0, "进阶： 是否使用 ScrollLock 灯来显示 CapsLockX 状态（不建议")
    global T_UseCapsLockLight := CapsLockX_Config("Core", "T_UseCapsLockLight", 0, "进阶： 是否使用 CapsLockX 灯来显示 CapsLockX 状态（强烈不建议")
    global T_SwitchSound := CapsLockX_Config("Core", "T_SwitchSound", 0, "进阶： 是否开启CapsLockX模式切换声音提示（默认不开）")
    global T_SwitchSoundOn := CapsLockX_Config("Core", "T_SwitchSoundOn", "./Data/NoteG.mp3", "CapsLockX 按下声音提示路径")
    global T_SwitchSoundOff := CapsLockX_Config("Core", "T_SwitchSoundOff", "./Data/NoteC.mp3", "CapsLockX 弹起声音提示路径")
    ; 不同模式下的拖盘图标
