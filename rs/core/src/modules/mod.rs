...
mod voice_ptt_test;
pub mod wake_word;
// Without `stt` (Windows builds, where whisper-rs 0.13 fails to compile)
// `VoiceModule` is the otoji-only implementation: same public surface, same
// Space+V behaviour, no in-process STT fallback.
#[cfg(not(feature = "stt"))]
#[path = "voice_lite.rs"]
pub mod voice;
