...
impl MediaModule {
    ...
    pub fn on_key_down(&self, key: KeyCode) -> bool {
        match key {
            ...
            KeyCode::F5 => {
                self.platform.key_tap_extended(KeyCode::MediaPlay);
                true
            }
            KeyCode::F6 => {
                self.platform.key_tap_extended(KeyCode::MediaPrev);
                true
            }
            KeyCode::F7 => {
                self.platform.key_tap_extended(KeyCode::MediaNext);
                true
            }
            KeyCode::F8 => {
                self.platform.key_tap_extended(KeyCode::MediaStop);
                true
            }
            KeyCode::F9 => {
                self.platform.key_tap_extended(KeyCode::VolumeUp);
                true
            }
            KeyCode::F10 => {
                self.platform.key_tap_extended(KeyCode::VolumeDown);
                true
            }
            KeyCode::F11 => {
                self.platform.key_tap_extended(KeyCode::VolumeMute);
                true
            }
            // Lianki shortcuts
            KeyCode::BracketLeft => {
