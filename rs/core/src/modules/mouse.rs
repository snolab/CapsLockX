...
impl MouseModule {
    ...
    pub fn apply_speeds(&self, s: &SpeedConfig) {
        self.mouse_model
            .set_ratios(s.mouse_speed, s.mouse_speed, f64::INFINITY);
        self.scroll_model
            .set_ratios(s.scroll_speed, s.scroll_speed, f64::INFINITY);
    }
    ...
    pub fn on_key_down(&self, key: KeyCode) -> bool {
        match key {
            KeyCode::W => {
                self.mouse_model.press_up();
                true
            }
            KeyCode::S => {
                self.mouse_model.press_down();
                true
            }
            KeyCode::A => {
                self.mouse_model.press_left();
                true
            }
            KeyCode::D => {
                self.mouse_model.press_right();
                true
            }
            KeyCode::Q => {
    ...
    pub fn on_key_up(&self, key: KeyCode) -> bool {
        match key {
            KeyCode::W => {
                self.mouse_model.release_up();
                true
            }
            KeyCode::S => {
                self.mouse_model.release_down();
                true
            }
            KeyCode::A => {
                self.mouse_model.release_left();
                true
            }
            KeyCode::D => {
                self.mouse_model.release_right();
                true
            }
            KeyCode::Q => {
                if self.right_btn.swap(false, Ordering::Relaxed) {
...
mod tests {
    ...
    fn q_press_emits_right_button_down() {
        ...
        assert!(plat
            .calls()
            .contains(&Call::MouseButton(MouseButton::Right, true)));
    }
    ...
    fn q_release_emits_right_button_up() {
        ...
        assert!(plat
            .calls()
            .contains(&Call::MouseButton(MouseButton::Right, false)));
    }
    ...
    fn e_press_emits_left_button_down() {
        ...
        assert!(plat
            .calls()
            .contains(&Call::MouseButton(MouseButton::Left, true)));
    }
    ...
    fn e_release_emits_left_button_up() {
        ...
        assert!(plat
            .calls()
            .contains(&Call::MouseButton(MouseButton::Left, false)));
    }
    ...
    fn shift_held_with_wasd_clamps_movement_to_unit() {
        ...
        for _ in 0..200 {
            sleep(Duration::from_millis(10));
            let calls = plat.calls();
            if let Some(Call::MouseMove(dx, dy)) =
                calls.iter().find(|c| matches!(c, Call::MouseMove(_, _)))
            {
                assert!(
                    dx.abs() <= 1 && dy.abs() <= 1,
                    "expected ±1 clamp, got ({},{})",
                    dx,
                    dy
                );
                ok = true;
