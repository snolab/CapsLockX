...
const CORPUS: &[Sample] = &[
    ...
    Sample {
        lang: "en",
        voice: "Samantha",
        text: "Please open the project folder and check the latest commit messages.",
    },
    Sample {
        lang: "en",
        voice: "Samantha",
        text: "Artificial intelligence research advances rapidly across many fields.",
    },
    Sample {
        lang: "ja",
        voice: "Kyoko",
        text: "今日は天気が良いので公園まで散歩に行きます。",
    },
    Sample {
        lang: "ja",
        voice: "Kyoko",
        text: "新しい本を図書館で借りて電車の中で読みました。",
    },
    Sample {
        lang: "ja",
        voice: "Kyoko",
        text: "明日の会議の資料はメールで送ってください。",
    },
    Sample {
        lang: "zh",
        voice: "Tingting",
    ...
    for (idx, s) in CORPUS.iter().enumerate() {
        ...
        match run_sample(s, &cache) {
            Ok(row) => {
                ...
                let cell =
                    |p: &Option<PathResult>| p.as_ref().map(fmt).unwrap_or_else(|| "—".into());
                eprintln!(
                    "    audio={:.1}s  A={}  B={}  C={}  D={}",
                    row.audio_secs,
                    fmt(&row.path_a),
...
fn run_sample(s: &Sample, cache: &Path) -> Result<Row, String> {
    ...
    let tts_ms = if wav.exists() {
        ...
        run(
            "say",
            &["-v", s.voice, "-o", &aiff.to_string_lossy(), s.text],
        )
...
fn transcribe_via_openrouter(wav: &Path, key: &str) -> Result<(String, u128), String> {
    ...
    let model =
        std::env::var("VOICE_QA_CLOUD_MODEL").unwrap_or_else(|_| "google/gemini-2.5-flash".into());
    ...
    let raw = resp
        .into_string()
        .map_err(|e| format!("openrouter audio read: {e}"))?;
...
fn polish_via_openrouter(text: &str, key: &str) -> Result<String, String> {
    let model =
        std::env::var("VOICE_QA_POLISH_MODEL").unwrap_or_else(|_| "google/gemini-2.5-flash".into());
    ...
    let raw = resp
        .into_string()
        .map_err(|e| format!("openrouter read: {e}"))?;
    let v: Value =
        serde_json::from_str(&raw).map_err(|e| format!("openrouter json: {e} — raw: {raw}"))?;
    let content = v
        .pointer("/choices/0/message/content")
        .and_then(|v| v.as_str())
...
fn render_report(rows: &[Row]) -> String {
    ...
    out.push_str(&format!(
        "- **C** — OpenRouter audio (`{cloud_model}`, cloud STT)\n"
    ));
    ...
    out.push_str("|--:|------|-----------|-------|------:|------:|------:|------:|---------:|-----:|-----:|-----:|-----:|\n");
    let cer_cell = |p: &Option<PathResult>| {
        p.as_ref()
            .map(|x| format!("{:.1}%", x.cer * 100.0))
            .unwrap_or_else(|| "—".into())
    };
    let ms_cell = |p: &Option<PathResult>| {
        p.as_ref()
            .map(|x| x.elapsed_ms.to_string())
            .unwrap_or_else(|| "—".into())
    };
            ...
            if let Some(p) = p {
                if p.hypothesis.trim() != r.path_a.hypothesis.trim() {
                    out.push_str(&format!(
                        ...
                        i + 1,
                        label,
                        md_escape(&truncate(&p.hypothesis, 90))
                    ));
                    any_div = true;
                }
            }
    ...
    let mean_ms = |xs: Vec<u128>| -> String {
        if xs.is_empty() {
            ...
            format!("{}", xs.iter().sum::<u128>() / xs.len() as u128)
        }
    };
    out.push_str(&format!(
        "| A | {} | {} |\n",
        rows.len(),
        mean_ms(rows.iter().map(|r| r.path_a.elapsed_ms).collect())
    ));
    out.push_str(&format!(
        "| B | {} | {} |\n",
        ...
        mean_ms(
            rows.iter()
                .filter_map(|r| r.path_b.as_ref().map(|p| p.elapsed_ms))
                .collect()
        )
    ));
    out.push_str(&format!(
        ...
        mean_ms(
            rows.iter()
                .filter_map(|r| r.path_c.as_ref().map(|p| p.elapsed_ms))
                .collect()
        )
    ));
    out.push_str(&format!(
        ...
        mean_ms(
            rows.iter()
                .filter_map(|r| r.path_d.as_ref().map(|p| p.elapsed_ms))
                .collect()
        )
    ));
