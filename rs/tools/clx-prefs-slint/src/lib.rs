//! Rendered either standalone or inside `clx.exe` as `clx prefs-window`.
...
pub fn run(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    //
    // `is_enabled()` shells out to `schtasks /query` (~80-100 ms). Doing that
    // here, before `win.run()`, held the whole window back by that much on
    // every Space+, — so query it on a thread and let the checkbox catch up a
    // few frames after first paint.
    win.set_autostart_supported(cfg!(windows));
    win.set_autostart_hint(autostart_hint().into());
    {
        let win_weak = win.as_weak();
        std::thread::spawn(move || {
            let enabled = clx_autostart::is_enabled();
