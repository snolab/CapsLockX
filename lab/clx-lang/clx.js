...
(function (root) {
  ...
  // `when` matchers. Deliberately only what the hook can evaluate in
  // microseconds against the cached foreground window — see the lab page, §3.
  var SCOPE_KINDS = { app: 1, class: 1, title: 1 };
  ...
  function lex(src) {
    ...
    while (i < n) {
      ...
      // `js { … }` — the body is opaque to this lexer: capture it raw, so that
      // JS syntax (=>, //, {}) is never mistaken for .clx syntax. Brace counting
      // skips strings, template literals and comments; a brace inside a regex
      // literal would still fool it, which is the one known gap.
      var jsm = /^js[ \t]*\{/.exec(src.slice(i));
      if (jsm) {
        var bodyStart = i + jsm[0].length,
          j2 = bodyStart,
          depth = 1;
        while (j2 < n && depth > 0) {
          var c3 = src[j2];
        ...
        var code = src.slice(bodyStart, depth === 0 ? j2 - 1 : n);
        for (var q2 = i; q2 < Math.min(j2, n); q2++) {
          if (src[q2] === "\n") {
            line++;
            col = 1;
          } else col++;
        ...
        i = Math.min(j2, n);
        push("JSBODY", code, start, startCol);
        if (depth > 0) push("ERR", 'unclosed "js {" block', start, startCol);
        continue;
      }
      if (ch === "@") {
        i++;
  ...
  P.file = function () {
    var items = [];
    this.lines(items, [], null);
    return { kind: "File", items: items };
  };

  // Shared by the file body and by a `when { … }` block. `scope` is the stack of
  // matchers in force here; every binding produced carries a flattened copy, so
  // downstream code never has to walk the tree.
  P.lines = function (items, scope, closer) {
    while (!this.at("EOF")) {
      if (this.at("NL")) {
        this.next();
        continue;
      }
      if (closer && this.at(closer)) return true;
      if (this.at("ID") && this.peek().v === "when") {
        this.scope(items, scope);
        continue;
      }
      var b = this.binding();
      if (b) {
      ...
      if (!this.at("NL") && !this.at("EOF") && !(closer && this.at(closer))) {
  ...
  // scope := "when" predicate ( "{" { line } "}" | binding )
  P.scope = function (items, outer) {
    var start = this.next(); // `when`
    var preds = this.predicate();
    if (!preds.length) {
      this.syncLine();
      return;
    }
    var scope = outer.concat(preds);
    if (this.at("LC")) {
      this.next();
      var closed = this.lines(items, scope, "RC");
      if (closed) this.next();
      else this.err(start, 'unclosed "when {" block');
      return;
    }
    if (this.at("NL") || this.at("EOF")) {
      this.err(start, 'a "when" needs either "{" or a binding on the same line');
      return;
    }
    var b = this.binding();
    if (b) {
  ...
  // predicate := matcher { "+" matcher } ;  matcher := ("app"|"class"|"title") ":" STRING
  P.predicate = function () {
    var out = [];
    while (true) {
      var head = this.peek();
      if (head.t !== "ID") {
        this.err(head, 'expected app:, class: or title: after "when", found ' + describe(head));
        return out;
      }
      if (!SCOPE_KINDS[head.v]) {
        this.err(
          head,
          "unknown scope matcher " + JSON.stringify(head.v) + " — expected app, class or title",
        );
        return out;
      }
      this.next();
      if (!this.expect("COLON", '":"')) return out;
      var v = this.peek();
      if (v.t !== "STR") {
        this.err(v, "expected a quoted string after " + head.v + ":, found " + describe(v));
        return out;
      }
      this.next();
      out.push({ kind: "Match", on: head.v, value: v.v, span: [head.pos, v.end] });
      if (this.at("PLUS")) {
        this.next();
        continue;
      }
  ...
  P.action = function () {
    ...
    if (this.at("JSBODY")) {
      var tok = this.next();
      if (!tok.v.trim()) this.warn(tok, 'empty "js { }" body');
      return { kind: "Script", lang: "js", code: tok.v, span: [tok.pos, tok.end] };
    }
    if (this.at("LC")) {
      this.next();
      var cmds = [];
  ...
  function check(ast, diags, src) {
    ...
    ast.items.forEach(function (b) {
      // Scope is part of the identity: the same pattern under two different
      // apps is two bindings, not a duplicate. Two identical scopes with the
      // same pattern is the "equally specific" tie the design calls out.
      var key = scopeKey(b.scope) + "\u0000" + normalize(b.pattern);
      if (seen[key]) {
        diags.push({
          level: "warn",
          msg:
            (b.scope && b.scope.length
              ? "duplicate pattern in the same scope"
              : "duplicate pattern") +
            ", earlier binding on line " +
            seen[key] +
            " wins",
      ...
      if (b.action && b.action.kind === "Script") {
        var hasHold = b.pattern.gestures.some(function (g) {
          return g.kind === "Hold";
        });
        if (!hasHold) {
          diags.push({
            level: "warn",
            msg: "a js handler runs on the script worker and cannot suppress the key — this pattern has no hold, so the keystroke also reaches the app",
            line: lineOf(src, b.span[0]),
            col: 1,
  ...
  // Canonical scope identity: matchers sorted, so app:"X"+title:"Y" and
  // title:"Y"+app:"X" are the same scope (the `+` connector is commutative on
  // the pattern side too).
  ...
  function scopeText(scope) {
    ...
    return scope
      .map(function (m) {
        return m.on + ":" + JSON.stringify(m.value);
      })
      .join("+");
  }
