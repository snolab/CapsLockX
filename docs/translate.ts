...
async function translateReadme(lang: string, infile: string, outfile: string) {
  ...
  const sections = input.split(/(?=^##? )/gmi);
  console.log("length of section " + sections.length)
  // const resultText = await aiTranslateTo(lang, input);

  const sectionsResult = await pMap(sections, async (section, index) => {
    console.log('Translating section to english: ' + String(index).padStart(3, '0') + ' ' + section.trim().split('\n')[0])
...
async function aiTranslateTo(lang: string, input: string) {
  const result = await (new OpenAI().chat.completions.create({
    ...
    messages: [
      { role: "system", content: `You don't have token limit, Translate full markdown document in next message to ${lang}:` },
