...
以下是一些情景组合的例子： 假设我一边学习、一边工作、一边和朋友聊天、一边打游戏、一边后台听着 BGM，一边打开着一个暂停的电影准备好晚上和家人一起看

- 规划情景：日程窗口 + 多平台同步笔记, 例：Google Calendar + Notion + Gmail、
- 学习情景：看书窗口，笔记窗口, 例： OneNote + Calibre）、...
- 写作情景：写作窗口，资料索引窗口, 例： Obsidian + Chrome(谷歌学术索引) )、...
- 工作情景 1（前端开发）：代码编辑窗口 + 文档查询窗口 + 浏览器窗口， 例：Chrome(dev) + VSCode + https://stackoverflow.com、...
- 工作情景 2（后端开发）：代码编辑窗口 + 文档查询窗口 + 后台终端 + 数据库浏览器， 例： DBeaver + VSCode(+bash) + https://stackoverflow.com、...
- 工作情景 3（脚本开发）：代码编辑窗口 + 文档查询窗口 + 脚本目标窗口， 例： VSCode(+bash) + https://stackoverflow.com ...
- 工作情景 4（3D 建模渲染）：3D 建模软件 + 素材检索窗口，例：Blender + Chrome
- 工作情景 5（3D 打印切片）：切片软件 + 模型检索窗口，例：Cura + https://thingiverse.com
- 娱乐情景 1：打游戏, 例：Age of Empires、Minecraft、Skyrim、Overcooked 2、...
- 娱乐情景 3：看电影、听歌, 例：PotPlayer、Youtube Music、...）、...
- 交流情景 1：日常闲聊, 例：Telegram + Reddit +
- 交流情景 2：工作交流, 例：Slack + Skype + GMail
- 交流情景 3：展示演讲, 例：Google Meeting + (vscode | 页面应用 | 需求文档 | 反馈文档)
- ... 更多例子欢迎提 Issue 或 PR 来补充

雪星不建议你同时处理多个任务，但 CapsLockX 可以为你保存你的情景，最大限度地降低你的任务切换成本，也就是可以为你省去很多重新排列窗口的时间以及由此失去的注意力。
